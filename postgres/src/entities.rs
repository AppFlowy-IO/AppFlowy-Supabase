...
impl PostgresConfiguration {
  pub fn from_env() -> Result<Self, Error> {
    let url = std::env::var(SUPABASE_DB).map_err(|_| anyhow::anyhow!("Missing SUPABASE_DB"))?;
    let user_name =
      std::env::var(SUPABASE_DB_USER).map_err(|_| anyhow::anyhow!("Missing SUPABASE_DB_USER"))?;
